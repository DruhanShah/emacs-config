...
;; Svg-tag templates
;; -----------------------------------
(defconst date-re "[0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\} [A-Za-z]\\{3\\}")
(defconst time-re "[0-9]\\{2\\}:[0-9]\\{2\\}-?[0-9]\\{0,2\\}:?[0-9]\\{0,2\\}")
(defconst inc-re "\\+[0-9][A-Za-z]")
...
(setq svg-tag-tags
      `(
        ;; Active date (with or without day name, time or increment)
        (,(format "\\(<%s>\\)" date-re) .
         ((lambda (tag)
            (svg-tag-make tag :beg 1 :end -1 :margin 0))))
        (,(format "\\(<%s %s>\\)" date-re inc-re) .
         ((lambda (tag)
            (svg-tag-make tag :beg 1 :end -1 :margin 0))))
        ...
        (,(format "\\(<%s \\)%s %s>" date-re time-re inc-re) .
         ((lambda (tag)
            (svg-tag-make tag :beg 1 :inverse nil :crop-right t :margin 0))))
        (,(format "<%s \\(%s \\)%s>" date-re time-re inc-re) .
         ((lambda (tag)
            (svg-tag-make tag :inverse t :crop-left t :crop-right t :margin 0))))
        (,(format "<%s %s \\(%s>\\)" date-re time-re inc-re) .
         ((lambda (tag)
            (svg-tag-make tag :end -1 :inverse nil :crop-left t :margin 0))))

        ;; Inactive date  (with or without day name, time or increment)
        (,(format "\\(\\[%s\\]\\)" date-re) .
         ((lambda (tag)
        ...
        (,(format "\\[%s \\(%s \\)%s\\]" date-re time-re inc-re) .
         ((lambda (tag)
            (svg-tag-make tag :inverse t :crop-left t :crop-right t :margin 0 :face 'org-date))))
        (,(format "\\[%s %s \\(%s\\]\\)" date-re time-re inc-re) .
         ((lambda (tag)
        ...
        ("\\(:[A-Z]+\\)" .
         ((lambda (tag)
            (svg-tag-make tag :beg 1 :face 'org-tag :margin 0))))
        ("\\(:[A-Z]+:\\)$" .
         ((lambda (tag)
            (svg-tag-make tag :beg 1 :end -1 :face 'org-tag :margin 0))))
        ...
        ("DONE" .
         ((lambda (tag)
            (svg-tag-make "DONE" :face 'org-done :margin 0))))
        ("CANCEL" .
         ((lambda (tag)
            (svg-tag-make "CANCEL" :face 'org-done :margin 0))))
        ("WAIT" .
         ((lambda (tag)
            (svg-tag-make "WAIT" :face 'org-todo :inverse t :margin 0))))
