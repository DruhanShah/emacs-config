...
(defvar nano-elfeed-icon-path "~/.config/emacs/nano/icons")
...
(defvar nano-elfeed-rss-icon-inactive
      (nano-elfeed--make-icon
       (create-image (format nano-elfeed--rss-icon-data "gray") 'svg t)))
  ...
  (let* ((icon (or (nano-elfeed-get-icon title unread)
    ...
    (insert (concat
     ...
     (propertize " " 'face face-upper
                     'display '((raise 0.5) (space :width (2))))
     (propertize " " 'face face-upper 'display '(raise 0.8))
...
(defun nano-elfeed-search-mode ()
  ...
  (face-remap-add-relative 'hl-line :foreground nano-base :background nano-overlay0)
  (hl-line-mode t)
  )
...
(bind-key "k" #'nano-elfeed-show-prev 'elfeed-show-mode-map)
