...
;;; Commentary:
;; 
;; NΛNO Sidebar is a package for creating sideframes that are glued to another
...
;; `nano-sidebar-toggle-maximized` function that computes the size properly.
;;
;;
;; Usage example:
;;
;; (nano-sidebar-make 'left 32)
;; (nano-sidebar-make 'right 32)
;; (nano-sidebar-toggle-maximized)
;;
...
;;
;; (nano-sidebar-make 'left 32 'dark `((foreground-color . ,nano-dark-foreground)
...
(defcustom nano-sidebar-default-side 'left
...
(defcustom nano-sidebar-default-size 36
  "Default size."
  :type 'natnum
  :group 'nano-sidebar)


(defun nano-sidebar--update (&optional frame)
  ...
  (let* ((screen-width (nth 2 (frame-monitor-geometry)))
    ...
    (when frame-top
      (modify-frame-parameters frame-top `((top . 0)
                                           (left . 0))))
    (when frame-bottom
...
(defun nano-sidebar-toggle-maximized ()
  "Toggle the maximized state of a frame that has zero, one or several sideframes."
...
(defun nano-sidebar-make (&optional side size mode parameters)
  "Create a fixed size frame on given SIDE.
\(left, right, top or bottom\) with given MODE and SIZE.
...
NOTE: Right and bottom configuration require a dynamic update of their positions
      ...
      force updating, user can call again the nano-sidebar-make function, this will
      update position."
  ...
  (interactive
   (let* ((completion-ignore-case  t))
     (list (intern (completing-read "Side (default left): "
           ...
           (read-number "Size: " nano-sidebar-default-size)
           (intern (completing-read "Mode (default light): "
                            '(light dark) nil t "")))))

  (let* ((side (if (memq side '(left right top bottom))
                   side
                 nano-sidebar-default-side))
         (mode (if (memq mode '(light dark))
                   mode
                 nano-sidebar-default-mode))
         (size (if (> size 0)
                   size
                 nano-sidebar-default-size))
         ...
         (child-frame (cond ((eq side 'right)
                             (frame-parameter parent-frame 'sidebar-right))
                            ((eq side 'top)
                             (frame-parameter parent-frame 'sidebar-top))
                            ((eq side 'bottom)
                             (frame-parameter parent-frame 'sidebar-bottom))
                            (t ;; left
                             (frame-parameter parent-frame 'sidebar-left))))
    ...
    (cond ((eq side 'right)
           (set-frame-parameter parent-frame 'sidebar-right child-frame)
           (setq size-position `((height . 1.0)
                                 ...
                                 (left . ,parent-width))))
          ((eq side 'top)
           (set-frame-parameter parent-frame 'sidebar-top child-frame)
           (setq size-position `((height . ,child-height)
                                 ...
                                 (left . 0))))
          ((eq side 'bottom)
           (set-frame-parameter parent-frame 'sidebar-bottom child-frame)
           (setq size-position `((height . ,child-height)
                                 (min-height . ,child-height)
                                 (width . 1.0)
                                 (keep-ratio . (width-only . nil))
                                 ...
                                 (left . 0))))
          (t ;; left
           (set-frame-parameter parent-frame 'sidebar-left child-frame)
           (setq size-position `((height . 1.0)
                                 (width . ,child-width)
                                 (min-width . ,child-width)
                                 (keep-ratio . (height-only . nil))
    ...
    (add-hook 'window-configuration-change-hook #'nano-sidebar--update)
...
(defun nano-sidebar-toggle (&optional side)
  "Toggle the visibility of a side frame on given SIDE."
  (interactive
   (let* ((completion-ignore-case  t))
     (list (intern (completing-read "Side (default left): "
				    '(left right top bottom) nil t ))))))
