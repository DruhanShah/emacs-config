;;; nano-border.el --- One pixel border around buffers     -*- lexical-binding: t; -*-
...
line. This requires to have nano-modelne installed."
  ...
  (mapc #'face-remap-remove-relative nano-border-cookies)
  (let ((color nano-modeline-border-color))
    (setq nano-border-cookies
...
;;;###autoload
(defun nano-border-off ()
  "Remove borders from the current buffer."

  (setq-local nano-border-state nil
              left-fringe-width 1
              right-fringe-width 1
              left-margin-width 0
              right-margin-width 0
              mode-line-format nano-border-mode-line-format
...
(defun nano-border ()
  ...
  (if (not nano-border-state)
      (nano-border-on)
    (nano-border-off))
  (redisplay t))

(provide 'nano-border)
;;; nano-border.el ends here
